...
def is_base64_encoded(sample):
   '''Check if some text is base64-encoded'''
   base64_chars = string.letters + string.digits + '+/= \t\n\r'
   try:
      sample.decode('base64')
      if any([char not in base64_chars for char in sample]):
         return False
      else:
         return True
   except:
...
def is_hex_encoded(sample):
   '''Check if some text hex-decodes without error'''
   hex_chars = string.hexdigits + ' \t\n\r'
   try:
      sample.decode('hex')
      if any([char not in hex_chars for char in sample]):
         return False
      else:
         return True
   except:
...
def detect_polybius(ciphertext):
   '''Detect the use of the polybius cipher'''
   correct_charset = all([char in ' 01234567' for char in ciphertext])
   correct_length = len(filter(lambda x: x in '01234567',ciphertext)) % 2 == 0
...
def check_key_reuse(samples):
   '''
   Check for key reuse between two messages
   
   Returns a boolean indicating whether two messages have high or low bitwise correspondence,
   which suggests key reuse.
   '''
   if len(samples) == 1:
...
def detect_ecb(ciphertext):
   '''Attempts to detect use of ECB by detecting duplicate blocks'''
   ciphertext_len = len(ciphertext)
...
def derive_d_from_pqe(p,q,e):
   '''Given p, q, and e from factored RSA modulus, derive the private key'''
