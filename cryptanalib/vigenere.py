...
def break_shift(ciphertext, ref_letter_freq, correlation = False):
    ...
    if correlation:
        ...
        # return a list of shifts, sorted by descending cross correlation value
        ...
        # return a list of shifts, sorted by ascending chi square value
        ...
        for k in xrange(len(shifts)-1):
            if shifts[k+1][1] < 50:
                ...
                # if then step from this chi square valur to the next higher is too big,
                # export only the list until this value
...
def detect_plaintext(candidate_text, pt_freq_table=frequency.frequency_tables['english_icase_letters'], common_words=frequency.common_words['english'], only_single_letter=True, detect_words=False):
    if only_single_letter:
        # measure letter frequency
        freq = [0]*26
        ...
        if comparison_method == 'chi_square': # real solution is only at 1000/4000
        ...
        elif comparison_method == 'correlation': # real solution is at 4/4000 -> only [15 22]
            score = 0.0
            for k in xrange(26):
...
def break_vigenere(ciphertext, scan_range, num_answers=1, max_best_shifts=2, num_key_lengths=1, ref_letter_freq=english_letter_frequency):
    ...
    # only take three letter words (all two letter words were already covered in the digraph charset)
    ...
    for key_length in key_lengths:
        ...
        # now perform key_length times an ascii shift cipher breaker against all subblocks
        digits_shifts = []
        ...
        # now construct all possible compinations from these proposed shifts for every digit of the key
        digit_shift_index = [0]*key_length
        while True:
            # construct key from different shift possibilities for each digit
            ...
            keys[current_key] = detect_plaintext(plaintext.lower(), only_single_letter=True)
    ...
    # Check if the key is repeating: that would mean we wrongly picked a multiple of the real key length
    # Calculate a modified autocorrelation that only checks the equality between numbers
    #l = len(numeric_key)
    #autocorr = [sum([numeric_key[i]==numeric_key[(i+x) % l] for i in xrange(l)]) for x in xrange(l)]

    # now do another run on plaintext detection, this time with multigraph and word count,
    # this is very slow but more accurate.
    # first add all the keys with low key lenghts (there will not be many of them so that's okey)
    keys2 = []
    if len(key_lengths) > 1:
        key2 = filter(lambda tup: len(tup[0]) == min(key_lengths), keys_sorted_by_single_letter_score)

    for (current_key,score) in keys_sorted_by_single_letter_score[:100]:
        plaintext = translate_vigenere(ciphertext, current_key, decrypt=True)
        keys2.append((current_key, detect_plaintext(plaintext.lower(), only_single_letter=False, detect_words=True)))
        # if the likeliness of english text is too low, stop trying
        #if score > 80.0:
        #    break
    
    keys_sorted_by_multigraph_and_words_score = sorted(keys2, key=operator.itemgetter(1))

    key_list = list(zip(*keys_sorted_by_multigraph_and_words_score)[0])

    if len(key_list) > 1:
        first_len = len(key_list[0])
        second_len = len(key_list[1])
        # deal with key multiplication:
